...
func HelpFunc() {
	...
	fmt.Println()
	cmd.Printf("  -lc, --license-check <IP> \n\t \\__ ")
	desc.Println("Scan common ports on the domain")
	fmt.Println()
	cmd.Printf("  -sp, --single-port <domain> <port>\n\t \\__ ")
	desc.Println("Check License status for cPanel/WHM on the given IP")
	fmt.Println()
	cmd.Printf("  -ssl <domain>\n\t \\__ ")
	desc.Println("Check SSL certificate validity")
	fmt.Println()
	cmd.Printf("  -rdap <domain>\n\t \\__ ")
	...
	cmd.Print("  domain_analyzer -lc 192.168.1.1        ")
	desc.Println("Check cPanel/WHM license status on given IP ")
	cmd.Print("  domain_analyzer -ssl example.com       ")
	desc.Println("Check SSL certificate validity on example.com")
	fmt.Println()
